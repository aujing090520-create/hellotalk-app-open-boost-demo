...
function renderFriends() {
  ...
  return `${statusBar()}<section class="screen subpage"><header class="page-head">${backButton()}<h2>选择指定语伴</h2><span class="head-icon"></span></header><main class="sub-content friend-page-content"><p class="friend-page-tip">选择最多 10 位互关语伴；每位语伴独立展示 1 次。</p><input class="friend-search" placeholder="搜索互关语伴" aria-label="搜索互关语伴" /><div class="friend-list">${friendOptions.map(friend => `<button class="friend ${selected.has(friend.name) ? 'is-selected' : ''}" data-action="friend" data-name="${friend.name}" data-initial="${friend.initial}" data-class="${friend.className}" data-active="${friend.active}"><span class="avatar ${friend.className}">${friend.initial}</span><span class="friend-info"><strong>${friend.name}</strong><span>${friend.note}</span></span><i class="${friend.active ? 'online' : 'offline'}"></i><i class="friend-check">${selected.has(friend.name) ? '✓' : ''}</i></button>`).join('')}</div></main><footer class="friend-select-footer"><span class="friend-selection-summary">已选择 <b>${draft.length}</b> / 10 位<em>共 ${totalPrice} 币</em></span><button class="primary" data-action="friend-confirm" aria-label="确认选择，本次共 ${totalPrice} 币">确认选择</button></footer></section>`;
}
