...
function renderMain() {
  ...
  return `${statusBar()}<section class="screen main-screen">
    ...
    <main class="main-content">
      ${banner}
      ${hero}
      <section class="section"><div class="card material-card" data-rule-target="cover">${anchor('cover', '3')}<button class="material-row" data-action="photo-sheet"><i class="setting-thumb ${state.photoSelected ? (state.photoSource === 'avatar' ? 'photo is-avatar' : 'photo is-photo') : ''}"></i><span><small>照片</small><strong>${state.photoSelected ? (state.photoSource === 'avatar' ? '头像' : '照片') : '选择'}</strong></span><i class="chevron">›</i></button><button class="material-row" data-action="open-template"><i class="template-thumb style-${state.template}"></i><span><small>风格</small><strong>${styleNames[state.template]}</strong></span><i class="chevron">›</i></button></div></section>
      <section class="section"><div class="card delivery-card compact-delivery"><div class="card-title-line"><h3 class="setting-card-title">投放给谁</h3><button class="history-link compact-history" data-action="open-records" aria-label="查看投放记录">◷</button></div><div class="mode-grid primary-mode-grid compact-mode-grid"><div class="mode-option"><button class="mode-card compact-mode ${!isDesignatedOnly ? 'is-selected' : ''}" data-action="mode" data-mode="random"><i class="compact-mode-icon">↻</i><strong>${!isDesignatedOnly ? '<i class="mode-check">✓</i>' : ''}随机推荐</strong></button></div><div class="mode-option"><button class="mode-card compact-mode ${isDesignatedOnly ? 'is-selected' : ''}" data-action="mode" data-mode="designated" data-rule-target="designated">${anchor('designated', '4')}<i class="compact-mode-icon partner-icon" aria-hidden="true"></i><strong>${isDesignatedOnly ? '<i class="mode-check">✓</i>' : ''}仅指定语伴</strong></button><button class="mode-help" data-action="mode-help" data-help="designated" aria-label="查看指定语伴说明"></button>${state.modeHelp === 'designated' ? '<div class="mode-tip mode-tip-right" role="tooltip">选择与你互相关注的语伴，可一次选择最多 10 位</div>' : ''}</div></div>${!isDesignatedOnly ? `<div class="audience-setting minimal-audience"><strong>匹配人群</strong><button class="inline-help" data-action="mode-help" data-help="audience-default" aria-label="查看默认条件说明">?</button>${state.modeHelp === 'audience-default' ? '<div class="mode-tip minimal-tip" role="tooltip">优先推荐给语言互相匹配的人群</div>' : ''}<div class="audience-switch"><button class="${state.audience === 'default' ? 'is-selected' : ''}" data-action="audience" data-audience="default">默认</button><button class="${state.audience === 'custom' ? 'is-selected' : ''}" data-action="audience" data-audience="custom">自定义</button></div></div><button class="partner-addon minimal-partner" ${designatedProduct ? 'data-action="open-friends"' : 'disabled'}><span class="select-partner-icon" aria-hidden="true"><i>+</i></span><span><strong>${hasDesignated ? `已选 ${state.selectedFriends.length} 位语伴` : '添加指定语伴'}</strong>${hasDesignated ? '<small>可选增值</small>' : '<small>可选</small>'}</span><em>${designatedProduct?.price || 0} 币/人</em><i class="chevron">›</i></button>` : `<button class="partner-addon designated-only minimal-partner" ${designatedProduct ? 'data-action="open-friends"' : 'disabled'}><span class="select-partner-icon" aria-hidden="true"><i>+</i></span><span><strong>${hasDesignated ? `已选 ${state.selectedFriends.length} 位语伴` : '选择指定语伴'}</strong><small>仅展示给选择的语伴</small></span><em>${designatedProduct?.price || 0} 币/人</em><i class="chevron">›</i></button>`}</div></section>
    </main>
